...
with st.sidebar:
    st_lottie("https://assets3.lottiefiles.com/packages/lf20_A6VCTi95cd.json",height=150) 
    
with st.sidebar:
    opt = option_menu(
        menu_title="Menu",
        options=["About", "Upload", "Store", "Insights"],
        icons=["youtube", "upload", "table", "bar-chart-line-fill"],
        menu_icon="cast",
        default_index=0
    )
...
if opt == "About":
            ...
            st.caption("The application should have the following features:")
            st.write("1. Ability to input a YouTube channel ID and retrieve all the relevant data (Channel name, subscribers, total video count, playlist ID, video ID, likes,dislikes, comments of each video) using Google API.")
            st.write("2. Option to store the data in a MongoDB database as a data lake.")
            st.write("3. Ability to collect data for up to 10 different YouTube channels and store them in the data lake by clicking a button.")
            st.write("4. Option to select a channel name and migrate its data from the data lake to a SQL database as tables.")
            st.write("5. Ability to search and retrieve data from the SQL database using different search options, including joining tables to get channel details.")
            
            st.write('Tools and Libraries Used👨🏻‍💻 :blue[Python, MongoDB, PostgreSQL, Google API Client & Streamlit]')
            
            st.header("Benefits of youtube data harvesting")
            st.write("Using a Youtube scraper makes it easy, fast, and cheap to extract useful data from Youtube. Whether you’re a small business owner or part of the data department at a large organization, you can experience many benefits from scraping Youtube.")
            st.subheader("Channel data")
            st.write("When you scrape the page for a Youtube channel, you’ll get data related to subscribers, amount of videos, playlists, and more. On the about page, you’ll find the total number of views for the channel.")
            st.subheader("Video performance data")
            st.write("When you scrape the page for a specific video, you’ll receive the number of views, likes, dislikes, channel subscribers, comments, and more. While each of these metrics can be analyzed individually, it is important to keep the ration of the metrics in mind.")
            st.subheader("Sentiment data")
            st.write("Thankfully, many loyal fans, casual viewers, and new viewers leave thoughtful comments either in response to or about the video’s content.. Then, your comment section essentially becomes a source of data related to fan requests or different types of videos they would like to see. Scraping the comment section makes it easier to make your subscribers happy and provide relevant content.")
    ...
    with col1:
        ...
        if channel_id and page  == "Enter":
            st.success("You can now preview the json file or upload the file in MongoDB")
    ...
    with col1 :
        st.subheader("Simplify and analyze the migrated data from the selected questions to gain valuable insights 	:chart_with_upwards_trend: :")
        ...
        question_selected = st.selectbox("Select the question from below options :ballot_box_with_check:", options=question_list)
    ...
    if question_selected == "What are the top 10 most viewed videos and their respective channels?":
        ...
        st.table(df)
        fig = px.bar(df,
                     x=['Channel Name', 'Top view count'][1],
                     y=['Channel Name', 'Top view count'][0],
                     orientation='h',
                     color=['Channel Name', 'Top view count'][0]
                    )
        st.plotly_chart(fig,use_container_width=True)
    ...
    if question_selected == "What is the total number of views for each channel, and what are their corresponding " \
        ...
        st.table(df)
        fig = px.bar(df,
                     x=['Channel Name', 'View Count'][0],
                     y=['Channel Name', 'View Count'][1],
                     orientation='v',
